...
NOM_OUT="$(${INCUS_BIN} exec "${BUILDER}" -- /bin/sh -lc "/run/current-system/sw/bin/nix build ${NIX_FLAGS} --option substituters https://cache.nixos.org --no-link --print-out-paths nixpkgs#nix-output-monitor" | tr -d '\r')"
